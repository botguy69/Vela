...
async function ensureTakes(
  ...
  const plan = planTakes({
    ...
    listed,
    weexResp: pos.weex_resp ?? "",
    pricePrecision: spec.pricePrecision,
    quantityPrecision: spec.quantityPrecision,
    formatPx: formatWeexPx,
    formatQty: formatWeexQty,
    stopOverride,
  });
  if (plan.throughTp1 && !pos.tp1_hit) {
    pos.tp1_hit = true;
  ...
  if (plan.placeTp) {
    ...
    for (let i = 0; i < tps.length; i += 1) {
      const slice = slices[i] ?? (plan.afterTp1 ? slices[0] : undefined);
  ...
  notes.push(`${pos.weex_symbol} 1 SL @ ${stopPx.toFixed(4)} + ${ok} algo TP`);
  await stampSet();
}
        ...
        if (regime.hot) {
          ...
          for (let pick of pool) {
            ...
            if (wantPct >= 12) {
              ...
              if (stopPct > 0.02 || stopPct < 0.01) {
                whyNot.unshift(`${tag} 12% needs 1–2% stop (got ${(stopPct * 100).toFixed(2)}%)`);
                continue;
              }
              if (rr < 1.1) {
                whyNot.unshift(`${tag} 12% needs ≥1.1R (got ${rr.toFixed(1)}R)`);
                continue;
              }
            }
          ...
          for (const r of ready) {
            if (room <= 0) break;
            if (apiOk && !apiOk.has(r.sized.weexSymbol)) continue;
            if (SKIP_WEEX.has(r.sized.weexSymbol)) continue;
            if (r.sized.entryType === "limit" && openLimits >= 2) continue;
            batch.push({ sized: r.sized, spec: r.spec });
            if (r.sized.entryType === "limit") openLimits += 1;
            room -= 1;
            break;
          }
          ...
          if (!batch.length) {
            ...
            if (!sent.ok) {
              ...
              if (/not supported via the API/i.test(replies[0] ?? "")) {
                SKIP_WEEX.add(sized.weexSymbol);
              }
            } else {
              notes.push(
          ...
          const watchThink = pool
            ...
            .slice(0, 3)
            .map((s) => {
              const tag = `${s.weexSymbol.replace("USDT", "")} ${s.side}`.toLowerCase();
              const hit = whyUniq.find((w) => w.toLowerCase().startsWith(tag)) ?? "not armed this tick";
